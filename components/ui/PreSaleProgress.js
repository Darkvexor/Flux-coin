...
export default function PreSaleProgress() {
  ...
  useEffect(() => {
    transactionAPI.getPreSaleStats()
      ...
      .catch(() => setStats({ soldAmount: 2600000, remaining: 2400000 }));
  }, []);

  if (!stats) return <div className="h-20 animate-pulse bg-white/[0.02] rounded-2xl"></div>;

  const soldAmount = stats.soldAmount || 2600000;
  const percentage = ((soldAmount / TOTAL_PRESALE_SUPPLY) * 100).toFixed(1);
    ...
    <div>
      <div className="flex justify-between items-end mb-4">
        ...
        <div className="text-right">
          <p className="text-gray-400 text-sm">{soldAmount.toLocaleString()} of {TOTAL_PRESALE_SUPPLY.toLocaleString()} FLUX sold</p>
          <p className="text-amber-400 text-sm font-bold">2.6M FLUX purchased by early backers</p>
        </div>
      </div>
      ...
      <p className="text-gray-500 text-xs mt-3">
        {(TOTAL_PRESALE_SUPPLY - soldAmount).toLocaleString()} FLUX remaining at $1.00
      </p>
    </div>
  );
}